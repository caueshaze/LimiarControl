...
describe("area targeting integration", () => {
  ...
  it("previews a cylinder area using the same circular footprint as sphere", async () => {
    const { app, repository } = createApp();
    registerIntegrationRoutes(app, repository, { emit: vi.fn() });
    ...
    const response = await app.inject({
      ...
      url: "/integration/sessions/demo-session/targeting/area/preview",
      payload: {
        actionId: "area-cylinder-preview-1",
        combatantId: "cmb_1",
        shape: "cylinder",
        originCell: { x: 4, y: 4 },
        anchorCell: { x: 10, y: 10 },
    ...
    expect(response.json().affectedCells).toEqual(
  ...
  it("rejects cylinder area targeting when the center point is out of range", async () => {
    const { app, repository } = createApp();
    registerIntegrationRoutes(app, repository, { emit: vi.fn() });
    ...
    const response = await app.inject({
      ...
      url: "/integration/sessions/demo-session/targeting/area",
      payload: {
        actionId: "area-cylinder-range-1",
        combatantId: "cmb_1",
        shape: "cylinder",
        originCell: { x: 4, y: 4 },
        anchorCell: { x: 10, y: 10 },
  ...
  it("rejects cylinder area targeting when line of effect to the center is blocked", async () => {
    const { app, repository } = createApp();
    registerIntegrationRoutes(app, repository, { emit: vi.fn() });
  ...
  it("resolves a cylinder area with no affected targets", async () => {
    const { app, repository } = createApp();
    registerIntegrationRoutes(app, repository, { emit: vi.fn() });
    ...
    const response = await app.inject({
      ...
      url: "/integration/sessions/demo-session/targeting/area/preview",
      payload: {
        actionId: "area-cylinder-empty-1",
        combatantId: "cmb_1",
        shape: "cylinder",
        originCell: { x: 4, y: 4 },
        anchorCell: { x: 2, y: 2 },
    ...
    expect(response.json().affectedCells.length).toBeGreaterThan(0);
