...
class BaseSpellSchemaTests(unittest.TestCase):
    ...
    def test_accepts_explicit_targeting_requirements(self):
        spell = self._make_create(
        ...
        self.assertFalse(spell.requiresTargetSight)
        self.assertFalse(spell.requiresTargetEffect)
        self.assertFalse(spell.requiresPointSight)
        self.assertTrue(spell.requiresPointEffect)

    def test_targeting_requirements_remain_optional_for_legacy_records(self):
        spell = self._make_create()
        self.assertIsNone(spell.requiresTargetSight)
        self.assertIsNone(spell.requiresTargetEffect)
        self.assertIsNone(spell.requiresPointSight)
        self.assertIsNone(spell.requiresPointEffect)
...
class BaseSpellSerializerTests(unittest.TestCase):
    def test_to_base_spell_read_maps_all_fields(self):
        ...
        self.assertTrue(read.requiresPointEffect)
        ...
        # coverAppliesToSave is None on the make_base_spell fixture (no value set)
        self.assertIsNone(read.coverAppliesToSave)

    def test_to_base_spell_read_includes_cover_applies_to_save(self):
        spell = make_base_spell(cover_applies_to_save="physical")
        read = to_base_spell_read(spell)
        self.assertEqual(read.coverAppliesToSave, "physical")

    def test_to_base_spell_read_cover_none_rule(self):
        spell = make_base_spell(cover_applies_to_save="none")
        read = to_base_spell_read(spell)
        self.assertEqual(read.coverAppliesToSave, "none")
    ...
    def test_to_base_spell_seed_entry_roundtrip(self):
        ...
        self.assertFalse(entry.requiresTargetSight)
    ...
    def test_to_base_spell_seed_entry_includes_cover_applies_to_save(self):
        spell = make_base_spell(cover_applies_to_save="physical")
        entry = to_base_spell_seed_entry(spell)
        self.assertEqual(entry.coverAppliesToSave, "physical")
...
class BaseSpellSeedTests(unittest.TestCase):
    ...
    def test_write_and_read_seed_document_roundtrip(self):
        ...
        self.assertTrue(document.spells[0].requiresTargetSight)
        self.assertTrue(document.spells[0].requiresTargetEffect)
