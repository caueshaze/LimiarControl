...
class TestConfigSecurityValidation(unittest.TestCase):
    ...
    def test_production_passes_with_strong_secrets(self):
        ...
        s.minio_access_key = "strong-minio-access-key"
        s.minio_secret_key = "strong-minio-secret-key"
        s.limiar_map_internal_key = "strong-limiar-map-internal-key"
        # Should not raise
        s.validate_production_secrets()
...
class TestUploadSizeLimitSecurity(unittest.TestCase):
    def test_read_limited_raises_on_oversized_file(self):
        ...
        from app.services.media_storage_service import MAX_FILE_SIZE_BYTES
        ...
        fake = FakeUpload(MAX_FILE_SIZE_BYTES + 1)
        with self.assertRaises(HTTPException) as ctx:
            asyncio.run(_read_limited(fake, MAX_FILE_SIZE_BYTES))
