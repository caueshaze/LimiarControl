...
class CombatFlowTestsMixin:
    ...
    async def test_start_combat(
        ...
        self.assertEqual(new_state.map_selection["kind"], "demo_map")
        self.db.add.assert_called_once()
        self.db.commit.assert_called_once()
        mock_emit_state.assert_called_once()
    ...
    @patch("app.services.combat.CombatService._sync_all_participant_statuses")
    @patch("app.services.combat.CombatService.get_state", return_value=None)
    async def test_start_combat_persists_campaign_map_selection(
        self,
        _mock_get_state,
        mock_sync_all_participant_statuses,
        mock_emit_log,
        ...
        session_result.first.return_value = CampaignSession(
            id="session-123",
        ...
        campaign_map_result = MagicMock()
        campaign_map_result.first.return_value = CampaignTacticalMap(
            id="map-123",
            campaign_id="campaign-123",
            name="Dungeon Depths",
            image_url="/api/assets/campaigns/campaign-123/maps/asset1234567890abcdef1234567890ab",
            grid_width=32,
            grid_height=24,
            calibration_x=0.1,
            calibration_y=0.2,
        ...
        req = CombatStartRequest(
            ...
            selectedMap=CombatMapChoice(kind="campaign_map", mapId="map-123"),
        )

        new_state = await CombatService.start_combat(self.db, "session-123", req)

        self.assertEqual(new_state.map_selection["kind"], "campaign_map")
        self.assertEqual(new_state.map_selection["mapId"], "map-123")
        self.assertEqual(new_state.map_selection["mapName"], "Dungeon Depths")
        self.assertEqual(
            new_state.map_selection["imageUrl"],
            "/api/assets/campaigns/campaign-123/maps/asset1234567890abcdef1234567890ab",
        )
        self.assertEqual(new_state.map_selection["gridWidth"], 32)
        self.assertEqual(new_state.map_selection["gridHeight"], 24)
        self.assertEqual(
            new_state.map_selection["calibration"],
    ...
    @patch("app.services.combat.CombatService._emit_state")
    @patch("app.services.combat.CombatService._emit_log")
    async def test_attack_hit_and_miss(
        self, mock_emit_log, mock_emit_state, mock_emit_entity_hp_update
    ):
        self.state.phase = CombatPhase.active
        self.state.current_turn_index = 0
    ...
    async def test_attack_damage_applies_colossus_slayer_only_once_per_turn(
        ...
        self.assertTrue(
            self.state.participants[0]["turn_resources"]["colossus_slayer_used"]
        )
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    self.assertEqual(
                        first_result["save_success_outcome"], "half_damage"
                    )
    ...
    @patch("app.services.combat.CombatService._emit_state")
    @patch("app.services.combat.CombatService._emit_log")
    async def test_draconic_sorcerer_adds_elemental_affinity_bonus_once_to_damage(
        self,
        mock_emit_log,
        mock_emit_state,
        mock_emit_entity_hp_update,
        ...
        # Draconic Bloodline (white = cold) sorcerer, level 6, Charisma 16 (+3).
        # Elemental Affinity must add +3 to a matching-type spell once, before the
        # half-damage save reduction: (8 + 1 + 3) // 2 == 6.
        self.state.phase = CombatPhase.active
        self.state.current_turn_index = 0
        attacker_state = SessionState(
            id="state-player",
            ...
            state_json={
                ...
                "subclassConfig": {"draconicAncestry": "white"},
                "level": 6,
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    self.assertTrue(first_result["is_saved"])
                    self.assertTrue(first_result["elemental_affinity_eligible"])
                    self.assertEqual(first_result["elemental_affinity_bonus"], 3)
        ...
        # base 8 + upcast bonus 1 + affinity 3 = 12 rolled; halved on save => 6.
        self.assertEqual(second_result["base_effect"], 8)
        self.assertEqual(second_result["effect_bonus"], 1)
        self.assertEqual(second_result["damage"], 6)
        mock_apply_damage.assert_called_once_with(
            self.db,
            "enemy-123",
            "session_entity",
        ...
        final_log = mock_emit_log.await_args_list[-1].args[1]["message"]
        self.assertIn("Dano rolado 12", final_log)
        self.assertIn("dano aplicado 6", final_log)
    ...
    async def test_player_heal_spell_spends_slot_and_applies_healing(
        ...
        self.assertEqual(
            attacker_state.state_json["spellcasting"]["slots"]["1"]["used"], 1
        )
            ...
            with patch(
                ...
                def get_stats_side_effect(_db, ref_id, kind, _session_id="", **_kwargs):
                    ...
                    raise AssertionError(
                        f"Unexpected get_stats call for {ref_id}/{kind}"
                    )
    ...
    async def test_player_spell_effect_lifecycle_creates_and_clears_pending_effect(
        ...
        with patch(
            ...
            self.assertIsNotNone(first_result["pending_spell_id"])
            self.assertEqual(
                self.state.participants[0]["pending_attack"]["type"],
                "player_spell_effect",
            )
    ...
    async def test_hunters_mark_cast_creates_concentration_effects(
        ...
        self.assertEqual(
            self.state.participants[0]["active_effects"][0]["kind"], "spell_effect"
        )
        self.assertEqual(
            self.state.participants[0]["active_effects"][0]["metadata"][
                "marked_target_participant_id"
            ],
            "e1",
        )
        self.assertEqual(
            self.state.participants[1]["active_effects"][0]["kind"], "spell_effect"
        )
    ...
    async def test_animal_friendship_rejects_non_beast_targets_before_spending_slot(
        ...
        self.assertEqual(
            attacker_state.state_json["spellcasting"]["slots"]["1"]["used"], 0
        )
    ...
    @patch(
        ...
        return_value=SystemType.DND5E,
    )
    @patch(
        "app.services.combat.CombatService._get_session_entry",
        return_value=MagicMock(campaign_id="camp-1", party_id="party-1"),
    )
    @patch("app.services.combat.CombatService._emit_player_state_update")
    @patch("app.services.combat.CombatService._emit_entity_hp_update")
    @patch("app.services.combat.CombatService._emit_state")
    ...
    async def test_goodberry_cast_marks_inventory_refresh_and_grants_item(
        ...
        self.assertLess(
            expires_at - datetime.now(timezone.utc), timedelta(hours=24, minutes=1)
        )
        ...
        with (
            ...
            patch(
                ...
                return_value=(
                    inventory_entry,
                    bracelet_item,
                    bracelet_item.magic_effect_json,
                ),
            ),
            patch(
                "app.services.combat.CombatService._get_spell_catalog_entry_for_session",
            ...
            patch(
                "app.services.combat_service.spells.cast_target.resolve_saving_throw",
    ...
    @patch("app.services.combat.CombatService._emit_state")
    @patch("app.services.combat.CombatService._emit_log")
    async def test_next_turn_skips_dead_and_defeated(
        self, mock_emit_log, mock_emit_state
    ):
        self.state.phase = CombatPhase.active
        self.state.current_turn_index = 0
