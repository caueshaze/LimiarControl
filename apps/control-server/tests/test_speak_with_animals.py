"""Tests for Speak with Animals (Falar com Animais).

Covers:
- Seed contract: level=1, ritual, self-target, no concentration, persistent
"""
...
class SpeakWithAnimalsSeedTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        cls.entry = next(
            (s for s in spells if isinstance(s, dict) and s.get("canonicalKey") == "speak_with_animals"),
            None,
        )

    def test_entry_exists(self):
        self.assertIsNotNone(self.entry, "speak_with_animals not found in seed")
