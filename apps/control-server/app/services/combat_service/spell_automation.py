...
# Spell execution classification:
#   A. Generic direct pipeline — spell_attack, saving_throw, direct_damage, heal
#      No handler registration needed; resolution_type + spell metadata drive the pipeline.
#   B. Generic area pipeline — cone, cube, sphere, line, cylinder
#      Driven by target_mode + area spec in _SUPPORTED_AREA_SPELL_SPECS.
#   C. Special handler pipeline — spells requiring unique scripted behavior
#      Registered below with a concrete handler_name pointing to a method on this mixin.
#
# Only category C belongs in this registry. Categories A and B resolve through
# generic pipelines driven by catalog metadata, not by handler code.
class CombatSpellAutomationMixin:
    ...
    def _validate_spell_automation_target(
        ...
        *,
        ...
        if (
            cls._normalize_spell_automation_key(spell_canonical_key)
            != "animal_friendship"
        ):
            return
        ...
        session_entity = db.exec(
            select(SessionEntity).where(
                SessionEntity.id == target_participant["ref_id"]
            )
        ).first()
