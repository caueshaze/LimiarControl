...
from app.services.base_spell_seeds import import_base_spell_seed_file, export_base_spell_seed_document
...
def admin_sync_base_spells_seed(
    ...
    """Import spells from seed JSON file into database (upsert mode)."""
    result = import_base_spell_seed_file(session, replace=False)
    return BaseSpellSeedSyncResult(**result)


@router.post("/base-spells/export-seed", response_model=BaseSpellSeedSyncResult)
def admin_export_base_spells_seed(
    _user: User = Depends(require_system_admin),
    session: Session = Depends(get_session),
):
    """Export all spells from database to seed JSON file."""
    document = export_base_spell_seed_document(session)
    return BaseSpellSeedSyncResult(
        inserted=0,
        updated=len(document.spells),
        deactivated=0,
