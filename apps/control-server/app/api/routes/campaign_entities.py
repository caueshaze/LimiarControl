...
def _validated_entity_image_ref(
    *,
    campaign_id: str,
    image_url: str | None,
    ...
    if ref.campaign_id != campaign_id:
        raise HTTPException(
            status_code=400,
            detail="imageUrl must reference an asset in the same campaign",
        )
    if ref.kind == "entity_temp":
        try:
            assert_managed_asset_exists(ref)
        except FileNotFoundError as exc:
            raise HTTPException(status_code=400, detail="Temporary image asset not found") from exc
        return ref
    if allow_existing_final and ref.kind == "entity_final" and ref.entity_id == entity_id:
        try:
            assert_managed_asset_exists(ref)
        except FileNotFoundError as exc:
            raise HTTPException(status_code=400, detail="Entity image asset not found") from exc
        return ref
    raise HTTPException(
        status_code=400,
        detail="imageUrl must reference a compatible managed entity asset",
    )
...
def create_campaign_entity(
    ...
    source_image_ref = _validated_entity_image_ref(
    ...
    try:
        ...
        if promoted_image_url is not None:
            delete_managed_url_best_effort(promoted_image_url)
        raise
    session.refresh(entry)
    if source_image_ref is not None:
        delete_managed_url_best_effort(source_image_ref.url)
...
def update_campaign_entity(
    ...
    elif source_image_ref.kind == "entity_temp":
        final_ref = build_entity_asset_ref(campaign_id, entity_id, source_image_ref.asset_id)
        copy_object(source_image_ref, final_ref)
        promoted_image_url = final_ref.url
        entry.image_url = promoted_image_url
    else:
    ...
    try:
        ...
        raise
    session.refresh(entry)
    if source_image_ref is not None and source_image_ref.kind == "entity_temp":
        delete_managed_url_best_effort(source_image_ref.url)
    if previous_image_url != entry.image_url:
