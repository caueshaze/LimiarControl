...
class BaseSpellWrite(BaseModel):
    ...
    # "physical" → cover applies. "none" → cover does not apply. Null → fallback.
    ...
    # During migration these remain optional; when absent the combat targeting
    # layer uses a temporary legacy fallback helper instead.
...
class BaseSpellRead(BaseModel):
    ...
    # Automation metadata (computed at serialization time, not stored in DB)
