...
export const GmDashboardPage = () => {
    ...
    const [legacyPartyId, setLegacyPartyId] = useState<string | null>(requestedPartyId);
    const [legacyPartyResolved, setLegacyPartyResolved] = useState(Boolean(requestedPartyId) || !effectiveCampaignId);
    const {
        combatModeVisible,
        combatUiExpanded,
        combatUiActive,
    ...
    useEffect(() => {
        if (activeSession?.partyId) {
            setLegacyPartyId(activeSession.partyId);
            setLegacyPartyResolved(true);
            return;
        }
        if (requestedPartyId) {
            setLegacyPartyId(requestedPartyId);
            setLegacyPartyResolved(true);
            return;
        }
        if (!effectiveCampaignId) {
            setLegacyPartyId(null);
            setLegacyPartyResolved(true);
            return;
        }

        let cancelled = false;
        setLegacyPartyResolved(false);
        partiesRepo.listMine()
            .then((parties) => {
                if (cancelled) {
                    return;
                }
                const fallbackParty = parties.find((party) => party.campaignId === effectiveCampaignId) ?? null;
                setLegacyPartyId(fallbackParty?.id ?? null);
                setLegacyPartyResolved(true);
            })
            .catch(() => {
                if (cancelled) {
    ...
    }, [activeSession?.partyId, effectiveCampaignId, requestedPartyId]);

    useEffect(() => {
        if (!effectiveCampaignId || loading || activeSession || !legacyPartyResolved) {
            return;
        }
        if (legacyPartyId) {
            navigate(routes.partyDetails.replace(":partyId", legacyPartyId), { replace: true });
            return;
        }
        navigate(routes.campaignEdit.replace(":campaignId", effectiveCampaignId), { replace: true });
    }, [activeSession, effectiveCampaignId, legacyPartyId, legacyPartyResolved, loading, navigate]);
