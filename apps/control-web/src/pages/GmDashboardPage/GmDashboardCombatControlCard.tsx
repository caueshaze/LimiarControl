...
import { CombatStartModal, type CombatParticipantPreview, type CombatStartSelection } from "./CombatStartModal";
...
export const GmDashboardCombatControlCard = ({
  ...
  const handleConfirmCombatStart = async ({ participants: included, selectedMap }: CombatStartSelection) => {
    ...
    try {
      ...
      await combatRepo.startCombat(activeSessionId, {
        participants: parts as any,
        selectedMap,
      });
      onCommand("start_combat", { mapKind: selectedMap.kind, mapId: selectedMap.mapId ?? null });
  ...
  return (
    <>
      <div className="rounded-2xl border border-slate-800 bg-linear-to-br from-slate-950/60 to-slate-900/40 p-4">
        ...
        onConfirm={(selection) => {
          void handleConfirmCombatStart(selection);
        }}
      />
    </>
  );
