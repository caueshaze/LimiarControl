  ...
  return (
    ...
    >
      ...
      {invite.activeSession?.title && (
        <p className="mt-4 rounded-[18px] border border-white/8 bg-white/4 px-4 py-3 text-sm text-slate-200">
          {invite.activeSession.title}
        </p>
      )}
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <button
          type="button"
          onClick={onDecline}
          className="rounded-full border border-white/10 bg-white/4 px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.2em] text-slate-200 transition hover:border-white/20 hover:bg-white/8"
        >
