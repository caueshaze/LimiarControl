...
describe("createBaseSheetActions", () => {
  ...
  it("selectCatalogSpell writes campaignSpellId when catalog entry has one", () => {
    // Seed with a campaign-scope spell carrying a campaignSpellId
    seedSpellCatalogCache(
      [
        {
    ...
    let current: CharacterSheet = {
      ...
      spellcasting: {
        ...
        slots: { 1: { max: 2, used: 0 } },
        spells: [
          { id: "spell-slot-1", name: "", level: 0, school: "Evocation", prepared: false, notes: "", campaignSpellId: null },
        ],
      },
    };
    ...
    actions.selectCatalogSpell("spell-slot-1", "guidance");

    expect(current.spellcasting?.spells[0]?.campaignSpellId).toBe("camp-spell-1");
    expect(current.spellcasting?.spells[0]?.canonicalKey).toBe("guidance");
  });

  it("updateSpell preserves campaignSpellId when editing other fields", () => {
    let current: CharacterSheet = {
      ...INITIAL_SHEET,
      class: "cleric",
      spellcasting: {
    ...
    // Edit an unrelated field (notes)
    actions.updateSpell("spell-1", "notes", "Some note");

    expect(current.spellcasting?.spells[0]?.campaignSpellId).toBe("camp-spell-1");
    expect(current.spellcasting?.spells[0]?.notes).toBe("Some note");
  });
});
