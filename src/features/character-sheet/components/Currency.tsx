...
const toDraftValues = (currency: CharacterSheet["currency"]) => {
  const wallet = fromCopper(currency.copperValue);
  return {
    cp: String(wallet.cp),
    sp: String(wallet.sp),
    ep: String(wallet.ep),
    gp: String(wallet.gp),
    pp: String(wallet.pp),
  } satisfies Record<CurrencyUnit, string>;
};
...
export const Currency = ({ currency, setCurrency, readOnly = false }: Props) => {
  ...
  useEffect(() => {
    setDraftValues(toDraftValues(currency));
  }, [currency.copperValue]);

  const commitCoinValue = (coin: CurrencyUnit, rawValue: string) => {
    const trimmedValue = rawValue.trim();
    const nextValue = trimmedValue === "" ? 0 : Math.max(0, safeParseInt(trimmedValue));
    setDraftValues((current) => ({ ...current, [coin]: String(nextValue) }));
    setCurrency(coin, nextValue);
  };
        ...
        {COINS.map(({ key, label, accent }) => (
          <div key={key} className="flex flex-col gap-2">
            ...
            <input
              ...
              disabled={readOnly}
              onChange={(e) => setDraftValues((current) => ({ ...current, [key]: e.target.value }))}
              onBlur={(e) => commitCoinValue(key, e.target.value)}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
