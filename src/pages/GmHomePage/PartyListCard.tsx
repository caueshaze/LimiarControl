...
export const PartyListCard = ({
  ...
  return (
    <article
      onClick={onOpen}
      className={`group cursor-pointer rounded-[28px] border p-5 transition hover:-translate-y-px ${
        featured
    ...
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <span
          className={`shrink-0 rounded-full px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.22em] transition ${
            featured
              ? "border border-limiar-300/15 bg-limiar-400/10 text-limiar-50 group-hover:border-limiar-300/30"
              : "border border-white/8 bg-white/4 text-slate-400 group-hover:border-limiar-300/20 group-hover:text-limiar-100"
          }`}
        >
